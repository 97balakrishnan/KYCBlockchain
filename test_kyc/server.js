...
app.post('/compress-jsonpack',function(req,res) {

	console.log(req.body.json);
	var input = req.body.json;
	var packed = jsonpack.pack(input);
	console.log(packed);
...
app.post('/compress-deflate',function(req,res){
	...
	console.log("DEFLATE ALGORITHMS ");
	var deflated = zlib.deflateSync(input);
	console.log("Deflated bytes : "+deflated);


	console.log("Deflate string base64 : "+deflated.toString('base64'));
	console.log("Deflated string utf : "+deflated.toString('utf8').length);
	console.log("Deflated getString(): "+getString(deflated).length);
	//var inflated = zlib.inflateSync(deflated);
	//console.log("Inflated bytes : "+inflated);

	
	res.send(deflated.toString('utf8'));
...
app.post('/compress-brotli',function(req,res){
	...
	var input = req.body.json;
	   ...
	   var deflated = brotli.compress(input).toString();
	   console.log(deflated);

	var inflated = brotli.decompress(deflated);
	console.log('inflated : '+inflated.toString());
	console.log('inflated : '+inflated.toString('base64'));
  	//var deflatedEncrypt = CryptoJS.AES.encrypt(deflatedString,"mypassword").toString();
   	res.send(string);
